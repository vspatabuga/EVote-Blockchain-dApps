...
export default function AdminPanel({ contract }) {
  ...
  // State Management yang sudah disatukan dan benar
  const [allSessions, setAllSessions] = useState([]);
  const [activeSession, setActiveSession] = useState(null); // Menggunakan nama 'activeSession' agar lebih jelas
  ...
  // Fungsi tunggal untuk memuat semua data yang relevan
  ...
  const handleApiCall = async (method, endpoint, body = {}) => {
    try {
      ...
      await loadPanelData(); // Selalu refresh semua data
    } catch (err) {
  ...
  const handleCreateSession = (e) => {
    e.preventDefault();
    handleApiCall('POST', '/api/admin/sessions', { sessionName: newSessionName });
  };
  const handleActivateSession = (sessionId) => handleApiCall('PUT', `/api/admin/sessions/${sessionId}`);
  const handleDeleteSession = (sessionId) => {
    if (window.confirm(`Yakin ingin menghapus Sesi ID: ${sessionId}?`)) {
      handleApiCall('DELETE', `/api/admin/sessions/${sessionId}`);
    }
  };

  // Handler Aksi Sesi Aktif
  ...
  const handleAddCandidate = (e) => {
    ...
    handleApiCall('POST', '/api/admin/add-candidate', { candidateName: newCandidateName, sessionId: activeSession.id });
    setNewCandidateName('');
  };
  const handleAuthorizeVoters = () => {
    ...
    handleApiCall('POST', '/api/admin/authorize-voters', { walletAddresses, sessionId: activeSession.id });
  };
    ...
    <div className="bg-light p-4 rounded">
      ...
      <Card className="mb-4">
        ...
        <Card.Footer>
          <Form onSubmit={handleCreateSession}>
            <Row><Col xs={8}><Form.Control type="text" placeholder="Nama Sesi Baru" value={newSessionName} onChange={(e) => setNewSessionName(e.target.value)} required /></Col><Col xs={4}><Button variant="dark" type="submit" className="w-100">Tambah Sesi</Button></Col></Row>
          </Form>
        </Card.Footer>
      </Card>
      ...
      {activeSession ? (
          ...
          {activeSession.status !== 'Selesai' ? (
            <Row>
              <Col md={5} className="mb-3 mb-md-0">
                <Card><Card.Header>Manajemen Kandidat</Card.Header><Card.Body>
                  <ListGroup variant="flush" className="mb-3">{candidates.length > 0 ? candidates.map(c => <ListGroup.Item key={c.id}>ID: {c.id} - {c.name}</ListGroup.Item>) : <ListGroup.Item>Belum ada kandidat.</ListGroup.Item>}</ListGroup>
                  <Form onSubmit={handleAddCandidate}>
                    <Form.Group><Form.Control type="text" placeholder="Nama Kandidat Baru" value={newCandidateName} onChange={(e) => setNewCandidateName(e.target.value)} required disabled={activeSession.status !== 'Belum Dimulai'}/></Form.Group>
                    <Button variant="dark" type="submit" className="mt-2 w-100" disabled={activeSession.status !== 'Belum Dimulai'}>Tambah Kandidat</Button>
                  </Form>
                </Card.Body></Card>
              </Col>
              <Col md={7}>
                <Card><Card.Header>Otorisasi Pemilih</Card.Header><Card.Body>
                  <div style={{maxHeight: '200px', overflowY: 'auto', marginBottom: '15px'}}><Table striped bordered hover size="sm"><thead><tr><th>NIM</th><th>Alamat Wallet</th></tr></thead><tbody>{registeredVoters.map(v => <tr key={v.registrasi_id}><td>{v.nim}</td><td className="text-monospace small">{v.wallet_address}</td></tr>)}</tbody></Table>{registeredVoters.length === 0 && <p className="text-muted text-center">Belum ada pemilih mendaftar.</p>}</div>
            ...
            <Card className="mt-4 border-success">
              <Card.Header as="h5" className="bg-success text-white">Hasil Akhir Pemilihan</Card.Header><Card.Body>
                ...
                <ListGroup variant="flush" className="mt-3">
                  {[...candidates].sort((a, b) => b.voteCount - a.voteCount).map(c => {
                      ...
                      return (<ListGroup.Item key={c.id}><div className="d-flex justify-content-between fw-bold"><span>{c.name}</span><span>{c.voteCount} Suara ({percentage}%)</span></div><ProgressBar now={percentage} label={`${percentage}%`} className="mt-2" /></ListGroup.Item>)
                  })}
                </ListGroup>
              </Card.Body></Card>
          )}
        ...
        <Alert variant="info" className="mt-4">
          Tidak ada sesi yang sedang aktif. Silakan buat sesi baru atau aktifkan salah satu dari daftar di atas.
        </Alert>
      )}
    </div>
  );
